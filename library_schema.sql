...
create event delete_reserve_after_7_days
...
delimeter$$
create trigger new_log_after_borrow on library_log
...
create trigger new_log_after_reserve on library_log
...
create trigger update_log_return on library_log
...
create trigger delete_from_log_after_cancel_reserve on library_log
...
select user_id, book_copy_id, due_date, days_of_delay
from borrow
where (select(datediff(current_date, due_date)) as days_of_delay) > 0;
